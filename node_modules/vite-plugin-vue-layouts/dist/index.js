...
function normalizePath(path) {
  ...
  return import_path.posix.normalize(path);
}
  ...
  for (const dir of dirsArray) {
    if (dir.includes("**")) {
      ...
      for (const match of matches)
        dirsResolved.push(normalizePath2((0, import_path2.resolve)(root, match)));
    } else {
      dirsResolved.push(normalizePath2((0, import_path2.resolve)(root, dir)));
    }
  }
  return dirsResolved;
}
...
function getImportCode(files, options) {
  ...
  for (const __ of files) {
    for (const file of __.files) {
      ...
      const parsed = (0, import_path3.parse)(file);
      const name = (0, import_path3.join)(parsed.dir, parsed.name).replace(/\\/g, "/");
    ...
    async load(id) {
      if (id === MODULE_ID_VIRTUAL) {
        ...
        for (const dir of layoutDirs) {
          const layoutsDirPath = dir.substr(0, 1) === "/" ? normalizePath2(dir) : normalizePath2((0, import_path4.resolve)(config.root, dir));
